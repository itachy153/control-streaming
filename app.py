...
# Enlace de visualización web directa (Ideal para evitar problemas de caché)
URL_HOJA = "https://google.com"

try:
    # Leer directamente la tabla con formato UTF-8 universal
    df = pd.read_csv(URL_HOJA, encoding='utf-8')
    # Limpiar encabezados quitando comillas invisibles que mete Google
...
# Asegurar la existencia de las columnas pase lo que pase
...
with tab1:
    ...
    # Limpieza estricta de espacios en la columna principal
    df["CLIENTE_LIMPIO"] = df["CLIENTE"].astype(str).str.replace('"', '').str.strip()
    df_validos = df[(df["CLIENTE_LIMPIO"] != "") & (df["CLIENTE_LIMPIO"].str.lower() != "nan")]
        ...
        for index, row in df_filtrado.iterrows():
            ...
            with st.container(border=True):
                ...
                with col_info:
                    ...
                    moneda_str = str(row['MONEDA']).replace('"', '').replace('nan', '').strip()
                ...
                with col_accion:
                    ...
                    # Limpiar teléfono de comillas y decimales
                    tel_sucio = str(row['TELEFONO']).replace('"', '').strip()
                    tel_limpio = "".join(filter(str.isdigit, tel_sucio.split('.')[0]))
